...
## Note: the analyses below are based on the integrated object ##
...
normal_dcis_idc_deg_top8 <- hbca_dcis_ibc.combined_marker %>% filter(p_val_adj < 0.05) %>% group_by(cluster) %>% filter(pct.1 > 0.5) %>% filter(pct.2 < 0.5) %>% top_n(n=7, wt=avg_log2FC) #%>% filter(pct.1 > 0.8)
...
###--------Figure 1f--------###
all_cell$celltype_com <- all_cell$celltype_upd1
all_cell$celltype_com <- factor(all_cell$celltype_com, 
                                levels = c("Basal", "lumhr", "lumsec",  "Bcell", "NK_T", "Myeloid", "Fibroblast", "Vascular", "Lymphatic", "PeriVas"))

all_cell$tumor_normal <- ifelse(rownames(all_cell@meta.data) %in% rownames(tumor_obj_rm_meta_data), 
                                'aneuploid', 'diploid')
...
###--------Figure 1g--------### 
dcis_clinical <- read.csv('./dcis_project_tissuetype_0916.csv', header = T)
